...
(function(){
  ...
  CD._teamContribBreakdown = (teamName) => {
    ...
    // Build roster owner map across all teams (matches computeMatchContribution)
    ...
    let xi = 0, bench = 0;
    const playerPts = {}; // cleanKey -> unrounded accumulated contribution (mult baked in)
  ...
  // Per-match single-number credit for a team (used by Points by Match team header row).
  ...
  // show a soft warning banner + Resync control. Guard with a simple
  // "already registered" flag so double-wiring doesn't fire handlers twice.
  if(!window._cdRosterChangeWired){
    ...
    window.addEventListener('_cdRosterChanged', (ev) => {
      try {
        ...
        if(!form || form.style.display === 'none') return; // banner only relevant when the form is open
  ...
  // Inject / refresh the banner above gscFormBody. Called on the event and
  // on each re-render of the admin tab.
  CD._paintRosterStaleBanner = () => {
    const form = document.getElementById('gscFormBody');
    if(!form) return;
  ...
  // Resync = re-read live roomState/draftState teams and clear the flag.
  // The match entry form itself reads roomState lazily, so simply dismissing
  // the banner is enough to unblock saves once the admin has looked.
  CD.resyncRosterSnapshot = () => {
    try {
      // Force a fresh mirror from the module state (both bridges poll, so
      // window.roomState / window.draftState are current already).
      const rs = window.roomState || window.draftState || {};
      // Expose a trivial cache-clear hook for any consumer that wants it
  ...
  CD._renderAdminTools = () => `
    ...
    <div class="adm-card">
      ...
      <div class="adm-row" style="margin-bottom:10px;">
        ...
        <div class="adm-sm"><label class="adm-lbl" title="Changing the multiplier re-applies retroactively across the season.">XI Multiplier <span style="color:var(--mute);font-weight:500;font-size:10px;cursor:help;" title="Changing the multiplier re-applies retroactively across the season.">(?)</span></label><input type="number" id="saMultValue" class="adm-inp" min="0.5" max="5" step="0.1" value="1" title="Changing the multiplier re-applies retroactively across the season."></div>
      </div>
      <div class="adm-row">
    ...
    return `
      ...
      <div style="padding:20px 24px;border-radius:18px;background:${isStarted ? 'linear-gradient(135deg,rgba(182,255,60,0.12),rgba(46,91,255,0.08))' : 'var(--glass)'};border:1px solid ${isStarted?'rgba(182,255,60,0.3)':'var(--line-2)'};margin-bottom:18px;display:flex;justify-content:space-between;align-items:center;gap:14px;flex-wrap:wrap;">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          ...
          ${isAdmin && !isStarted && members.length >= 2 ? `<button onclick="window.startDraft()" style="padding:10px 20px;border-radius:9999px;background:linear-gradient(180deg,var(--lime-2),var(--lime));color:#000;border:none;font-weight:700;font-size:13px;cursor:pointer;box-shadow:0 6px 24px rgba(182,255,60,0.3);display:inline-flex;align-items:center;gap:6px;">${I('check',14)} Start Draft</button>` : ''}
        </div>
      </div>
  ...
  // Live draft pick — current picker, pool, snake-order peek.
  // Replaces the auction's live-block view. Uses draft state:
  //   rs.draftOrder      [{teamName, uid, round}, ...] — snake order
  //   rs.currentPickIndex integer cursor into draftOrder
  //   rs.players         player pool with status 'available' | 'drafted'
  CD.renderAuctionLive = () => {
    ...
    const myTeam = window.myTeamName || '';
    const setup = rs.setup || {};
    const isStarted = !!setup.isStarted;

    // --- Pre-draft state --------------------------------------------------
    if(!isStarted){
      return `
        <div class="cd-glass-2" style="padding:48px 24px;border-radius:22px;background:var(--glass-2);border:1px solid var(--line-2);text-align:center;">
          ...
          <div class="ed" style="font-size:32px;line-height:1;">Draft <span class="ed-i" style="color:var(--pink);">not started</span></div>
          <p style="margin-top:12px;color:var(--ink-2);font-size:14px;">${isAdmin ? 'Head to Setup and press Start Draft when your teams are in.' : 'Waiting for the admin to start the draft…'}</p>
        </div>
      `;
    }

    const order = Array.isArray(rs.draftOrder) ? rs.draftOrder : [];
    const idx = rs.currentPickIndex || 0;
    const totalPicks = order.length;
    const picksDone = idx;
    const picksLeft = Math.max(0, totalPicks - idx);
    const current = order[idx] || null;

    // --- Draft complete ---------------------------------------------------
    if(!current || idx >= totalPicks){
      return `
        <div class="cd-glass-2" style="padding:48px 24px;border-radius:22px;background:var(--glass-2);border:1px solid var(--line-2);text-align:center;">
          <div style="margin:0 auto 18px;width:60px;height:60px;border-radius:50%;background:rgba(182,255,60,0.15);display:flex;align-items:center;justify-content:center;color:var(--lime);">${I('check',32)}</div>
          <div class="ed" style="font-size:32px;line-height:1;">Draft <span class="ed-i" style="color:var(--lime);">complete</span></div>
          <p style="margin-top:12px;color:var(--ink-2);font-size:14px;">All ${totalPicks} picks made. Head to Teams to see the final squads.</p>
        </div>
      `;
    }

    const amCurrentPicker = current.teamName === myTeam || (current.uid && current.uid === window.user?.uid);
    const cCode = teamCode(current.teamName);
    const [c1, c2] = TEAM_COLORS[cCode] || ['#444','#222'];

    // Next 4 picks preview (the snake order)
    const upcoming = order.slice(idx + 1, idx + 5);

    // Available player pool (for the current picker to choose from)
    const available = players.filter(p => p.status === 'available').slice(0, 150);
    const selectedPid = window._cdPickSelected || '';
    const selPlayer = available.find(p => String(p.id) === String(selectedPid));

    return `
      <!-- Current pick hero -->
      <div style="position:relative;border-radius:28px;overflow:hidden;background:radial-gradient(ellipse 80% 60% at 70% 20%,rgba(255,45,135,0.25),transparent 60%),radial-gradient(ellipse 90% 70% at 20% 80%,rgba(46,91,255,0.35),transparent 60%),linear-gradient(135deg,${c1}30,${c2}50);border:1px solid var(--line-2);box-shadow:var(--sh-2);padding:${CD.state.isMobile ? 20 : 32}px;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;flex-wrap:wrap;gap:10px;">
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
            ${CD.Pill({tone:'pink', children: CD.LiveDot() + ' ON THE CLOCK'})}
            ${CD.Pill({children: 'Round ' + (current.round || 1)})}
            ${amCurrentPicker ? CD.Pill({tone:'lime', children:'YOUR PICK'}) : ''}
          </div>
          <div style="font-size:11px;color:var(--mute);letter-spacing:0.14em;text-transform:uppercase;font-weight:700;">
            Pick ${idx + 1} <span style="color:var(--ink-2);">/ ${totalPicks}</span>
          </div>
        </div>
        <div style="display:grid;grid-template-columns:${CD.state.isMobile ? '1fr' : '1.5fr 1fr'};gap:${CD.state.isMobile ? 18 : 32}px;align-items:center;">
          <div>
            <div style="font-size:10px;color:var(--mute);letter-spacing:0.18em;text-transform:uppercase;font-weight:700;">Picking now</div>
            <div style="display:flex;align-items:center;gap:14px;margin-top:10px;">
              ${CD.Avatar({name: current.teamName, size: CD.state.isMobile ? 52 : 72})}
              <div style="min-width:0;">
                <div class="ed" style="font-size:${CD.state.isMobile ? 34 : 56}px;line-height:0.95;letter-spacing:-0.02em;overflow:hidden;text-overflow:ellipsis;">${esc(current.teamName)}</div>
                <div style="font-size:11px;color:var(--ink-2);margin-top:6px;letter-spacing:0.14em;font-weight:700;text-transform:uppercase;">${(teams[current.teamName]?.roster?.length || 0)} picked so far</div>
              </div>
            </div>
          </div>
          <div style="text-align:${CD.state.isMobile ? 'left' : 'right'};">
            <div style="font-size:10px;color:var(--mute);letter-spacing:0.18em;text-transform:uppercase;font-weight:700;">${picksLeft} picks left</div>
            <div style="font-family:var(--display);font-size:${CD.state.isMobile ? 48 : 72}px;line-height:0.85;font-weight:800;background:linear-gradient(180deg,#fff,var(--lime));-webkit-background-clip:text;-webkit-text-fill-color:transparent;filter:drop-shadow(0 0 30px rgba(182,255,60,0.3));">
              ${picksDone}<span style="font-size:${CD.state.isMobile ? 22 : 32}px;color:var(--mute);-webkit-text-fill-color:var(--mute);"> / ${totalPicks}</span>
            </div>
            <div style="margin-top:12px;height:6px;border-radius:3px;background:rgba(255,255,255,0.1);overflow:hidden;${CD.state.isMobile ? '' : 'max-width:240px;margin-left:auto;'}">
              <div style="height:100%;width:${totalPicks ? (picksDone/totalPicks*100).toFixed(1) : 0}%;background:linear-gradient(90deg,var(--electric),var(--lime));"></div>
            </div>
          </div>
        </div>
      </div>

      <!-- Pick controls + upcoming -->
      <div style="display:grid;grid-template-columns:${CD.state.isMobile ? '1fr' : '1.6fr 1fr'};gap:20px;margin-top:20px;">
        <div class="cd-glass-2" style="padding:20px;border-radius:22px;background:var(--glass-2);border:1px solid var(--line-2);">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
            <div style="font-size:10px;color:var(--mute);letter-spacing:0.18em;text-transform:uppercase;font-weight:700;">${amCurrentPicker || isAdmin ? 'Choose a player' : 'Waiting for pick…'}</div>
            ...
            ${selPlayer ? `
              <div style="padding:12px;border-radius:12px;background:linear-gradient(135deg,rgba(46,91,255,0.12),rgba(255,45,135,0.08));border:1px solid var(--line-2);margin-bottom:10px;">
                <div style="display:flex;align-items:center;gap:10px;">
                  ${CD.Avatar({team: selPlayer.iplTeam || selPlayer.t, name: selPlayer.name || selPlayer.n, size: 40})}
                  <div style="flex:1;min-width:0;">
                    <div style="font-weight:700;font-size:14px;text-overflow:ellipsis;overflow:hidden;white-space:nowrap;">${esc(selPlayer.name || selPlayer.n)}</div>
                    <div style="display:flex;gap:4px;margin-top:3px;flex-wrap:wrap;">
                      ${CD.TeamCodePill(selPlayer.iplTeam || selPlayer.t, 'font-size:9px;')}
                      ${CD.Pill({style:'font-size:9px;padding:1px 6px;', children: esc(selPlayer.role || selPlayer.r || '')})}
                      ${(selPlayer.isOverseas || selPlayer.o) ? CD.Pill({tone:'gold', style:'font-size:9px;padding:1px 6px;', children:'★ OS'}) : ''}
                    </div>
                  </div>
                </div>
              </div>
            ` : ''}
            <button onclick="CD.handleDraftPick()" ${!selectedPid ? 'disabled' : ''} style="width:100%;padding:14px;border-radius:9999px;background:${selectedPid ? 'linear-gradient(180deg,var(--lime-2,var(--lime)),var(--lime))' : 'rgba(255,255,255,0.08)'};border:none;color:${selectedPid ? '#000' : 'var(--mute)'};font-weight:800;font-size:14px;cursor:${selectedPid ? 'pointer' : 'not-allowed'};display:inline-flex;align-items:center;justify-content:center;gap:8px;">${I('check',15)} Lock in pick</button>
          ` : `
            <div style="padding:24px 18px;text-align:center;color:var(--mute);font-size:13px;border:1px dashed var(--line-2);border-radius:12px;">
              ${esc(current.teamName)} is on the clock. Your turn will come — the order is on the right.
            </div>
          `}
        </div>

        <div class="cd-glass-2" style="padding:20px;border-radius:22px;background:var(--glass-2);border:1px solid var(--line-2);">
          <div style="font-size:10px;color:var(--mute);letter-spacing:0.18em;text-transform:uppercase;font-weight:700;margin-bottom:12px;">Up next</div>
          ${upcoming.length ? upcoming.map((o, i) => {
            const c = teamCode(o.teamName);
            const [g1,g2] = TEAM_COLORS[c] || ['#444','#222'];
            const isMe = o.teamName === myTeam || (o.uid && o.uid === window.user?.uid);
            return `<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--line);">
              <div style="width:24px;text-align:center;font-family:var(--display);font-size:13px;font-weight:800;color:var(--mute);">${idx + 2 + i}</div>
              <div style="width:24px;height:24px;border-radius:50%;background:linear-gradient(135deg,${g1},${g2});box-shadow:inset 0 0 0 1px rgba(255,255,255,0.15);flex-shrink:0;"></div>
              <div style="flex:1;min-width:0;font-size:13px;font-weight:${isMe ? 700 : 500};color:${isMe ? 'var(--lime)' : 'var(--ink)'};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${esc(o.teamName)}${isMe ? ' (you)' : ''}</div>
              <div style="font-size:10px;color:var(--mute);letter-spacing:0.08em;font-weight:700;">R${o.round || '?'}</div>
            </div>`;
          }).join('') : '<div style="color:var(--mute);font-size:12px;padding:14px 0;text-align:center;">End of draft order</div>'}
        </div>
      </div>
    `;
  };
  ...
  // currentPickIndex increment, roster update, etc.
  CD.handleDraftPick = () => {
    const pid = window._cdPickSelected;
    if(!pid){ window.showAlert?.('Select a player first.','err'); return; }
    const sel = document.getElementById('playerSelect') || document.getElementById('pickPlayerSelect');
    if(sel){ sel.value = pid; }
    if(typeof window.lockInPick === 'function'){
      window._cdPickSelected = '';
      window.lockInPick(pid);
    } else {
      window.showAlert?.('Pick handler not ready — try again in a second.','err');
    }
  };

  // Teams (draft equivalent of "Purses") — each franchise's current roster.
  // Drafts don't have money, so the budget progress bar is replaced with a
  // "picks filled" indicator and an overseas counter.
  CD.renderPurses = () => {
    const rs = window.roomState || {};
    const teams = rs.teams || {};
    const isAdmin = window.isAdmin || false;
    ...
    if(!arr.length) return `<div style="padding:40px;border-radius:18px;background:var(--glass);border:1px solid var(--line);color:var(--mute);text-align:center;backdrop-filter:blur(20px);"><div style="margin:0 auto 12px;width:48px;height:48px;border-radius:50%;background:rgba(255,255,255,0.05);display:flex;align-items:center;justify-content:center;color:var(--mute);">${I('users',24)}</div><div class="ed" style="font-size:20px;margin-bottom:4px;">No teams yet</div><div style="font-size:13px;color:var(--mute);">Teams will appear here once they register.</div></div>`;
    ...
    return `
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(340px,1fr));gap:16px;">
        ${arr.map(t => {
          ...
          const pct = maxPlayers > 0 ? Math.min(100, (roster.length / maxPlayers) * 100) : 0;
            ...
            <div style="border-radius:20px;overflow:hidden;border:1px solid var(--line-2);background:var(--glass);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);box-shadow:var(--sh-1);position:relative;${isMyTeam ? 'outline:2px solid var(--lime);outline-offset:-2px;' : ''}">
              <div style="padding:18px 18px 16px;background:linear-gradient(135deg,${c1}55,${c2}35);position:relative;border-bottom:1px solid var(--line);">
                ...
                <div style="display:flex;align-items:center;gap:12px;position:relative;">
                  ...
                  <div style="flex:1;min-width:0;">
                    <div class="ed" style="font-size:20px;line-height:1.05;font-weight:800;letter-spacing:-0.02em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${esc(t.name)}</div>
                    <div style="margin-top:4px;display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
                      ${CD.Pill({children: roster.length + ' / ' + maxPlayers + ' picks'})}
                      ${overseas ? CD.Pill({tone:'gold', style:'font-size:10px;padding:2px 7px;', children: overseas + ' / ' + maxOs + ' OS'}) : ''}
              ...
              <div style="padding:14px 18px;border-bottom:1px solid var(--line);">
                <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:6px;">
                  <span style="font-size:10px;color:var(--mute);letter-spacing:0.14em;text-transform:uppercase;font-weight:700;">Roster filled</span>
                  <span style="font-family:var(--mono);font-size:11px;color:var(--mute);">${pct.toFixed(0)}%</span>
                </div>
              ...
              <div style="padding:14px 18px 18px;">
                <div style="font-size:10px;color:var(--mute);letter-spacing:0.14em;text-transform:uppercase;font-weight:700;margin-bottom:10px;">Roster</div>
                ${roster.length ? `
                ...
                ` : `<div style="padding:16px 0;color:var(--mute);font-size:12px;text-align:center;">No picks yet</div>`}
              </div>
            </div>
          `;
        }).join('')}
  ...
  // Pick history (draft equivalent of "Bid ledger") — every completed pick
  // in chronological (snake) order. CSV export mirrors the auction ledger.
  CD.renderLedger = () => {
    const rs = window.roomState || {};
    const order = Array.isArray(rs.draftOrder) ? rs.draftOrder : [];
    const idx = rs.currentPickIndex || 0;
    ...
    // Build a flat, ordered list of picks. The draftOrder drives the sequence.
    // For each completed slot, try to find the matching player that landed on
    // that team (status=drafted and roster index matches pick order).
    const completed = [];
    const teamRosterCursor = {};
    for(let i = 0; i < idx && i < order.length; i++){
      const slot = order[i];
      if(!slot) continue;
      const t = teams[slot.teamName];
      const roster = t ? (Array.isArray(t.roster) ? t.roster : Object.values(t.roster || {})) : [];
      const cur = teamRosterCursor[slot.teamName] || 0;
      const pick = roster[cur] || null;
      teamRosterCursor[slot.teamName] = cur + 1;
      completed.push({ order: i + 1, round: slot.round || 1, teamName: slot.teamName, player: pick });
    }

    const search = (window._cdLedgerSearch || '').toLowerCase().trim();
    const buyerFilter = window._cdLedgerBuyer || '';
    let filtered = completed.slice();
    if(search) filtered = filtered.filter(r => (r.player?.name || r.player?.n || '').toLowerCase().includes(search) || (r.teamName||'').toLowerCase().includes(search));
    if(buyerFilter) filtered = filtered.filter(r => r.teamName === buyerFilter);

    const buyers = [...new Set(completed.map(r => r.teamName))].sort();
    const totalPicks = order.length;

    if(!completed.length) return `<div style="padding:40px;border-radius:18px;background:var(--glass);border:1px solid var(--line);color:var(--mute);text-align:center;backdrop-filter:blur(20px);"><div style="margin:0 auto 12px;width:48px;height:48px;border-radius:50%;background:rgba(255,255,255,0.05);display:flex;align-items:center;justify-content:center;color:var(--mute);">${I('gavel',24)}</div><div class="ed" style="font-size:22px;margin-bottom:4px;">No picks <span class="ed-i" style="color:var(--pink);">yet</span></div><div style="font-size:13px;">Completed picks will appear in this log as the draft runs.</div></div>`;

    return `
      <!-- Stats -->
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px;margin-bottom:18px;">
        ${CD.Stat({val: completed.length, lbl:'Picks made', accent:'var(--electric)'})}
        ${CD.Stat({val: Math.max(0, totalPicks - completed.length), lbl:'Picks left', accent:'var(--pink)'})}
        ${CD.Stat({val: buyers.length, lbl:'Franchises', accent:'var(--lime)'})}
        ${CD.Stat({val: Math.max(...completed.map(r => r.round)) || 1, lbl:'Round', accent:'var(--gold)'})}
      </div>
      ...
      <div style="padding:12px 16px;border-radius:14px;background:var(--glass);border:1px solid var(--line);margin-bottom:14px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
        ...
        <select onchange="window._cdLedgerBuyer=this.value;CD.render();" style="padding:9px 12px;font-size:12px;color:var(--ink);background:var(--glass);border:1px solid var(--line-2);border-radius:9999px;min-width:140px;">
          <option value=""${buyerFilter === '' ? ' selected' : ''}>All franchises</option>
          ...
          <table style="width:100%;border-collapse:collapse;font-size:13px;">
            <thead>
              <tr style="background:rgba(0,0,0,0.2);">
                <th style="padding:10px 14px;text-align:left;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.14em;color:var(--mute);">Pick</th>
                <th style="padding:10px 14px;text-align:left;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.14em;color:var(--mute);">Round</th>
                <th style="padding:10px 14px;text-align:left;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.14em;color:var(--mute);">Franchise</th>
            ...
            <tbody>
              ${filtered.slice().reverse().map(r => {
                ...
                return `<tr style="border-top:1px solid var(--line);transition:background 0.15s;" onmouseover="this.style.background='rgba(255,255,255,0.03)'" onmouseout="this.style.background='transparent'">
                  <td style="padding:11px 14px;font-family:var(--display);font-size:15px;color:var(--mute);">${r.order.toString().padStart(3,'0')}</td>
                  <td style="padding:11px 14px;"><span style="padding:2px 8px;border-radius:9999px;background:rgba(255,255,255,0.05);border:1px solid var(--line);font-family:var(--mono);font-size:11px;color:var(--ink-2);">R${r.round}</span></td>
                  <td style="padding:11px 14px;"><div style="display:flex;align-items:center;gap:8px;">${CD.Avatar({name: r.teamName, size: 24})}<span style="font-weight:600;font-size:12.5px;">${esc(r.teamName)}</span></div></td>
                  <td style="padding:11px 14px;"><div style="display:flex;align-items:center;gap:10px;">${CD.Avatar({team: p.iplTeam || p.t, name, size: 30})}<span style="font-weight:600;">${esc(name)}${isOs ? ' <span style="color:var(--gold);font-size:10px;">★</span>' : ''}</span></div></td>
  ...
  CD.downloadLedgerCSV = () => {
    ...
    const teams = rs.teams || {};
    const teamCursor = {};
    const rows = [['Pick','Round','Franchise','Player','IPL Team','Role','Overseas']];
    for(let i = 0; i < idx && i < order.length; i++){
      const slot = order[i]; if(!slot) continue;
      const t = teams[slot.teamName];
      const roster = t ? (Array.isArray(t.roster) ? t.roster : Object.values(t.roster || {})) : [];
      const cur = teamCursor[slot.teamName] || 0;
      const p = roster[cur] || {};
      teamCursor[slot.teamName] = cur + 1;
      rows.push([i+1, slot.round || 1, slot.teamName, p.name || p.n || '', p.iplTeam || p.t || '', p.role || p.r || '', (p.isOverseas || p.o) ? 'Yes' : 'No']);
    }
    if(rows.length === 1){ window.showAlert?.('No picks made yet.','err'); return; }
    ...
    if(typeof window.showAlert === 'function') window.showAlert('Pick history CSV downloaded','ok');
  };
  ...
  CD.renderMyTeam = () => {
    ...
    // auto-pad — the user has to choose which reserves promote to XI.
    ...
    const renderTeamBlock = (tt) => {
      ...
      const rows = rostr.map(p => {
        ...
        matchIds.forEach(mid => {
          ...
          // else: reserves / not in snap → 0×, render — with 0 contribution.
      ...
      // Team header total — single source of truth so leaderboard == all-squads == here.
  ...
  // Note: `pts` is derived from the canonical CD._teamSeasonTotal helper so the leaderboard header
  // can NEVER diverge from the All-Squads subtotals or the Points-by-Match team header — even if
  // the stored `leaderboardTotals.pts` happens to be momentarily stale (the silent auto-heal pass
  // will fix the stored value on the next tick).
  ...
  CD._renderTeamRoster = (t, c1, c2) => {
    ...
    const playerPts = brk.playerPts; // cleanKey -> unrounded
  ...
  CD.renderTrades = () => {
    ...
    // classic #trades-tab form). State persists on CD._tradeForm so it
    // survives CD.render() innerHTML rebuilds. ──
    ...
    const partnerOpts = Object.values(teams)
      ...
      .join('');
    // Available = roster minus already-staged picks
...
`;
  ...
  // State lives on CD._tradeForm so it survives CD.render() rebuilds.
  CD._tradeForm = CD._tradeForm || { partner:'', send:[], receive:[] };
  CD.tradeSetPartner = (val) => {
    CD._tradeForm.partner = val || '';
    // Partner changed → previously-staged receive picks are no longer valid
  ...
  CD.tradeSubmit = () => {
    const tf = CD._tradeForm || {};
    // Draft app exposes cdSubmitTradeD; auction exposes cdSubmitTradeA.
  ...
  CD.handleCreateRoom = () => {
    ...
    if(typeof window.createDraftRoom === 'function') window.createDraftRoom();
  ...
  CD.handleJoinRoom = () => {
    ...
    if(cd && cl) cl.value = cd.value;
    if(typeof window.initiateJoin === 'function') window.initiateJoin();
  };
  ...
  CD._captureFormState = () => {
    try {
      ...
      // and auth* sign-in — a Firebase snapshot mid-type would wipe those.
  ...
  CD.hook = () => {
    ...
    setInterval(() => {
      try {
        ...
        // FNV-1a so the key string stays compact even with many matches
        let mh = 2166136261;
  ...
  function init(){
    ...
    if(window.user && window.user.uid){
      ...
      // URL has ?draft= — likely going to a room after auth restores; show splash, not auth screen
      CD.state.view = 'auth';
  ...
  if(!window.copyInviteLink){
    window.copyInviteLink = function(){
      const url = window.location.origin + window.location.pathname + '?draft=' + encodeURIComponent(window.roomId || '');
